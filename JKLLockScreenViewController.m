...
- (void)viewDidLoad {
    ...
    switch (_lockScreenMode) {
        ...
        case LockScreenModeNew: {
            // [신규 모드]
            [self lsv_updateTitle:NSLocalizedStringFromTable(@"Pin Code Required",    @"JKLockScreen", nil)
...
- (void)lsv_unlockScreenFailure {
    ...
    [_subtitleLabel setText:NSLocalizedStringFromTable(@"Pin Code Not Matched", @"JKLockScreen", nil)];
    ...
    dispatch_after(delayInSeconds, dispatch_get_main_queue(), ^(void){
        ...
        switch (_lockScreenMode) {
            ...
            case LockScreenModeChange:
                // [변경 모드]
                [self lsv_updateTitle:NSLocalizedStringFromTable(@"New Pin Code Required",    @"JKLockScreen", nil)
                             subtitle:NSLocalizedStringFromTable(@"", @"JKLockScreen", nil)];
                break;
            default:
                break;
...
- (void)lockScreenPincodeView:(JKLLockScreenPincodeView *)lockScreenPincodeView pincode:(NSString *)pincode {
    ...
    if (_lockScreenMode == LockScreenModeNormal) {
        ...
        // 재입력 타이틀로 전환
        [self lsv_updateTitle:NSLocalizedStringFromTable(@"Pin Code Required Confirm",    @"JKLockScreen", nil)
                     subtitle:NSLocalizedStringFromTable(@"", @"JKLockScreen", nil)];
...
#ifdef DEBUG
    NSLog(@"Entered Pincode : %@", _pincodeView);
#endif
