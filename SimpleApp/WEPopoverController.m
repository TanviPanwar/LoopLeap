...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)theContext {
	...
	if ([animationID isEqual:@"FadeIn"]) {
		...
		if (userInitiatedDismissal) {
			...
			[popoverdelegate popoverControllerDidDismissPopover:self];
		}
	}
}
...
- (void)presentPopoverFromRect:(CGRect)rect 
	...
	backgroundView.touchdelegate = self;
...
- (void)viewWasTouched:(WETouchableView *)view {
	if (popoverVisible) {
		if (!popoverdelegate || [popoverdelegate popoverControllerShouldDismissPopover:self]) {
