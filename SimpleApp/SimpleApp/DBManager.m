...
{
    char *errMsg;
    const char *sql_stmt = "create table if not exists MyDatabase (DatabaseId integer , DBName varchar, Owner_Userid INTEGER, Catalog text, LastModified varchar , DBType text , StorageType text)";
    if (sqlite3_exec(database, sql_stmt, NULL, NULL, &errMsg)
        != SQLITE_OK)
    {
        isSuccess = NO;
...
{
    char *errMsg;
    const char *sql_stmt = "create table if not exists SharedDatabases (DatabaseId integer, UserId integer, Admin text, importDBFlag integer)";
    if (sqlite3_exec(database, sql_stmt, NULL, NULL, &errMsg)
        != SQLITE_OK)
    {
        isSuccess = NO;
...
-(BOOL) createNewDataBase_dataBaseId:(NSString*)dataBaseId dbName:(NSString*)dbName ownerUser_Id:(NSString*)ownerUser_Id cataLog:(NSString*)cataLog lastModified:(NSString*)lastModified dbType:(NSString*)dbType storageType:(NSString*)storageType
{
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        ...
        NSString *insertSQL = [NSString stringWithFormat:@"insert into MyDatabase (DatabaseId, DBName , Owner_Userid, Catalog, LastModified , DBType , StorageType) values (\"%d\",\"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\")", [dataBaseId intValue], dbName, ownerUser_Id, cataLog, lastModified ,dbType , storageType];
...
-(BOOL) saveDataInMyDatabaseFromServer:(MyDataBaseObject*)myDataObj
{
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        ...
        NSString *insertSQL = [NSString stringWithFormat:@"insert into MyDatabase (DatabaseId, DBName , Owner_Userid, Catalog, LastModified , DBType ) values (\"%lld\",\"%@\", \"%lld\", \"%@\", \"%@\", \"%@\")", myDataObj.Databaseid,myDataObj.DBName,myDataObj.Owner_Userid,myDataObj.Catelog,myDataObj.LastModified , myDataObj.DBType];
...
-(NSMutableArray*)getDefaultDBData {
    NSMutableArray * data = [[NSMutableArray alloc] init];
    data = [[DBManager getSharedInstance] defaultDBData: [NSString stringWithFormat:@"select DatabaseId , DBName from MyDatabase ORDER BY DatabaseId ASC LIMIT 1"]];
    return data;
}
...
-(BOOL) saveDataInShareDBTableFromServer:(ShareDBObject*)dbObj
{
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        ...
        NSString *insertSQL = [NSString stringWithFormat:@"insert into SharedDatabases (DatabaseId, UserId , Admin) values (\"%lld\",\"%lld\", \"%lld\")", dbObj.DatabaseID,dbObj.UserID,dbObj.Admin];
...
//    return NO;
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        const char *sqliteQuery = "update user_notes SET title = ?, note = ?, imageData = ?  where Notes_id =?  and DatabaseId= ?";
        ...
        if( sqlite3_prepare_v2(database, sqliteQuery, -1, &statement, NULL) == SQLITE_OK )
        {
            ...
            sqlite3_bind_text(statement, 4, [notesID UTF8String], -1, SQLITE_TRANSIENT);
            sqlite3_bind_text(statement, 5, [DBID UTF8String], -1, SQLITE_TRANSIENT);
            
            NSUInteger data;
            data = [imgData length];
            sqlite3_bind_blob(statement, 3, [imgData bytes], data, SQLITE_TRANSIENT);
            sqlite3_step(statement);
        }
